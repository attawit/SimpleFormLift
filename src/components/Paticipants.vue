...
export default {
  ...
  mounted() {
    // Set the initial number of registerList
    this.totalRows = this.registerList.length;
  },
  created() {
    // Binding Collections
     ...
     this.$dbevents
      ...
      .onSnapshot((snapshot) => {
        snapshot.docChanges().forEach((change) => {
          ...
          this.$set(this.registerList, ioldRegister, getRegister);
        });
      });
  ...
  methods: {
    ...
    updatePaticipant() {
      this.$dbpaticipants
        .doc(this.paticipant.id)
        .update(this.paticipant)
        .then(() => {
          //this.refreshTable();
          this.$refs.edit.hide();
          toast.fire({
    ...
    deletePaticipant(doc) {
      Swal.fire({
        ...
        if (result.value) {
          this.$dbpaticipants.doc(doc.id).delete();
